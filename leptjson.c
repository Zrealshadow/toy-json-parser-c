...
void lept_move(lept_value *dst, lept_value *src)
{
    ...
    lept_free(dst);
    memcpy(dst, src, sizeof(lept_value));
    lept_init(src); /* Ownership transfer*/
}
...
lept_value *lept_insert_array_element(lept_value *v, size_t index)
{
    ...
    /*
    move elements
    dst_addr = &v->u.a.e[index + 1]; src_addr = &v->u.a.e[index];
    count = v->u.a.size - index;
    */
...
void lept_erase_array_element(lept_value *v, size_t index, size_t count)
{
    ...
    if (count + index > v->u.a.size)
    {
        /* no need to copy left elements, all are erased*/
        v->u.a.size = index;
    }
    else
    {
        lept_value *dst_addr;
        const lept_value *src_addr;
        dst_addr = &v->u.a.e[index];
        src_addr = &v->u.a.e[index + count];
        memmove(dst_addr, src_addr, (v->u.a.size - index - count) * sizeof(lept_value));
        v->u.a.size -= count;
    }
}
...
lept_value *lept_set_object_value(lept_value *v, const char *key, size_t klen)
{
    size_t index;
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    /* first check the key is existed */
